...
test("renders the Avento Motors home page", async () => {
  ...
  assert.match(html, /class="footer-phone"/);
...
test("ships the finished visual system without starter artifacts", async () => {
  ...
  assert.match(css, /\.footer-phone:hover \{[\s\S]*color: #fff;/);
  ...
  assert.match(site, /calculateMonthlyPayment\(car\.price\)/);
  assert.match(site, /prefetchGallery/);
  assert.match(site, /onPointerEnter=\{prefetchGallery\}/);
  ...
  assert.match(css, /@media \(max-width: 768px\)[\s\S]*\.hero-carousel \{[\s\S]*height: clamp\(246px, 64vw, 310px\);/);
